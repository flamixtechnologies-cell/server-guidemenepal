...
const registerGuide = asyncHandler(async (req, res) => {
  ...
  const liscensePhoto = req.files?.liscensePhoto
    ? req.files.liscensePhoto[0].path
    : null;
  const certificationPhoto = req.files?.certificationPhoto
    ? req.files.certificationPhoto[0].path
    : null;
  ...
  let liscenseURL, profileURL, selfVideoURL, certificationURL, galleryURLs;

  if (liscensePhoto) {
    const uploadResponse = await uploadOnCloudinary(
      liscensePhoto,
      "/guide-details/liscense"
    );
  ...
  if (certificationPhoto) {
    const uploadResponse = await uploadOnCloudinary(
      certificationPhoto,
      "/guide-details/certification"
    );
    certificationURL = uploadResponse?.secure_url || null;
  }
  if (profilePhoto) {
    const uploadResponse = await uploadOnCloudinary(
      profilePhoto,
      "/guide-details/profile"
    );
  ...
  if (gallery.length > 0) {
    const uploadResponses = await Promise.all(
      gallery.map((file) => uploadOnCloudinary(file, "/guide-details/gallery"))
    );
    galleryURLs = uploadResponses.map((res) => res?.secure_url || null);
  }
  ...
  const guide = await Guide.create({
    ...
    certificationPhoto: certificationURL,
    selfVideo: selfVideoURL,
    gallery: galleryURLs || [],
    guidingAreas: JSON.parse(guidingAreas) || [],
    guideDestinations: JSON.parse(guideDestinations) || [],
    aboutMe,
